...
bool	renumber = yes		{prompt="Renumber output apertures?"}
int	offset = 0		{prompt="Output aperture number offset"}
bool	clobber = yes		{prompt="Modify existing output images?"}
bool	merge = no		{prompt="Merge with existing output images?"}
bool	rebin = no		{prompt="Rebin to exact wavelength region?"}
